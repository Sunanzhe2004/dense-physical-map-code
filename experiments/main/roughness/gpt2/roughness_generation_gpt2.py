...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate roughness maps with GPT Image 2 from RGB-only, RGB+example, or RGB+seg inputs."
    )
    ...
    parser.add_argument("--watermark", action="store_true", help="Compatibility flag; ignored for GPT Image 2")
...
def ensure_api_key(cli_api_key: Optional[str] = None) -> str:
    ...
    raise RuntimeError(
        ...
        "AZURE_ROUGHNESS_OPENAI_API_KEY / AZURE_GPT_IMAGE_2_API_KEY / AZURE_OPENAI_API_KEY / OPENAI_API_KEY."
    )
...
def normalize_gpt_image_size(size: Optional[str]) -> str:
    ...
    if normalized not in SUPPORTED_GPT_IMAGE_SIZES:
        raise ValueError(
            f"Unsupported --size for gpt-image-2: {size}. "
