...
RUN_SCRIPT="${SCRIPT_ROOT}/depth_generation_qwen.py"
...
COMMON_ARGS=(
  ...
  --image_model "${IMAGE_MODEL:-wan2.7-image}"
  --size "${IMAGE_SIZE:-1280*960}"
  --skip_existing
)

if [[ "${GENERATION_MODE:-}" != "" ]]; then
  COMMON_ARGS+=(--generation_mode "${GENERATION_MODE}")
fi
...
for ((worker_id = 0; worker_id < WORKERS; worker_id++)); do
  key_var="DASHSCOPE_API_KEY_${worker_id}"
  worker_api_key="${!key_var:-${DASHSCOPE_API_KEY:-}}"
  if [[ -z "${worker_api_key}" ]]; then
    echo "${key_var} or DASHSCOPE_API_KEY is not set." >&2
    exit 1
  fi
  ...
  (
    set +e
    export DASHSCOPE_API_KEY="${worker_api_key}"
    status=0
...
if (( status == 0 )); then
  echo "All depth qwen jobs completed."
else
  echo "Some depth qwen jobs failed. Check ${LOG_DIR}/worker_*.log." >&2
fi
exit "${status}"
