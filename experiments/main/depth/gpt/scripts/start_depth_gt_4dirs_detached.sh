...
has_key=0
for var_name in AZURE_GPT_IMAGE_15_API_KEY AZURE_GPT_IMAGE_2_API_KEY AZURE_OPENAI_API_KEY OPENAI_API_KEY; do
  if [[ -n "${!var_name:-}" ]]; then
    has_key=1
  fi
done
for idx in 0 1 2 3; do
  ...
  var_name_gpt2="AZURE_GPT_IMAGE_2_API_KEY_${idx}"
  if [[ -n "${!var_name:-}" || -n "${!var_name_gpt2:-}" ]]; then
    has_key=1
  fi
done
if (( has_key == 0 )); then
  echo "AZURE_GPT_IMAGE_15_API_KEY/AZURE_GPT_IMAGE_2_API_KEY/AZURE_OPENAI_API_KEY/OPENAI_API_KEY or AZURE_GPT_IMAGE_15_API_KEY_0..3 / AZURE_GPT_IMAGE_2_API_KEY_0..3 is not set." >&2
  exit 1
fi
...
setsid env \
  ...
  AZURE_GPT_IMAGE_2_API_KEY_0="${AZURE_GPT_IMAGE_2_API_KEY_0:-}" \
  AZURE_GPT_IMAGE_2_API_KEY_1="${AZURE_GPT_IMAGE_2_API_KEY_1:-}" \
  AZURE_GPT_IMAGE_2_API_KEY_2="${AZURE_GPT_IMAGE_2_API_KEY_2:-}" \
  AZURE_GPT_IMAGE_2_API_KEY_3="${AZURE_GPT_IMAGE_2_API_KEY_3:-}" \
  AZURE_GPT_IMAGE_15_ENDPOINT="${AZURE_GPT_IMAGE_15_ENDPOINT:-}" \
  AZURE_GPT_IMAGE_2_ENDPOINT="${AZURE_GPT_IMAGE_2_ENDPOINT:-}" \
  AZURE_GPT_IMAGE_15_API_VERSION="${AZURE_GPT_IMAGE_15_API_VERSION:-}" \
  AZURE_GPT_IMAGE_2_API_VERSION="${AZURE_GPT_IMAGE_2_API_VERSION:-}" \
