...
RUN_SCRIPT="${SCRIPT_ROOT}/depth_generation_seed.py"
INPUT_ROOT="${INPUT_ROOT:-${GT_ROOT:-/path/to/benchmark_data/GT}}"
BASE_OUTPUT_DIR="${BASE_OUTPUT_DIR:-/path/to/benchmark_outputs/depth_doubao}"
...
for ((worker_id = 0; worker_id < WORKERS; worker_id++)); do
  key_var="ARK_API_KEY_${worker_id}"
  worker_api_key="${!key_var:-${ARK_API_KEY:-}}"
  if [[ -z "${worker_api_key}" ]]; then
    echo "${key_var} or ARK_API_KEY is not set." >&2
    exit 1
  fi
  ...
  (
    set +e
    export ARK_API_KEY="${worker_api_key}"
    status=0
...
if (( status == 0 )); then
  echo "All depth doubao jobs completed."
else
  echo "Some depth doubao jobs failed. Check ${LOG_DIR}/worker_*.log." >&2
fi
exit "${status}"
