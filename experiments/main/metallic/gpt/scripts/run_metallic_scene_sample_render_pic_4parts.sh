...
RUN_SCRIPT="${RUN_SCRIPT:-${SCRIPT_ROOT}/metallic_generation_gpt.py}"

INPUT_DIR="${INPUT_DIR:-/path/to/benchmark_data/metallic_scene}"
OUTPUT_DIR="${OUTPUT_DIR:-/path/to/benchmark_outputs/metallic_gpt}"
...
API_KEY="${AZURE_METALLIC_OPENAI_API_KEY:-${AZURE_GPT_IMAGE_15_API_KEY:-${AZURE_OPENAI_API_KEY:-${OPENAI_API_KEY:-}}}}"
API_ENDPOINT="${AZURE_METALLIC_OPENAI_ENDPOINT:-${AZURE_GPT_IMAGE_15_ENDPOINT:-${AZURE_OPENAI_ENDPOINT:-}}}"
API_VERSION="${AZURE_METALLIC_OPENAI_API_VERSION:-${AZURE_GPT_IMAGE_15_API_VERSION:-${AZURE_OPENAI_API_VERSION:-2025-04-01-preview}}}"
if [[ -z "${API_KEY}" ]]; then
  echo "GPT Image API key is not set. Set one of AZURE_METALLIC_OPENAI_API_KEY / AZURE_GPT_IMAGE_15_API_KEY / AZURE_OPENAI_API_KEY / OPENAI_API_KEY." >&2
  exit 1
fi
if [[ -z "${API_ENDPOINT}" ]]; then
  echo "GPT Image endpoint is not set. Set one of AZURE_METALLIC_OPENAI_ENDPOINT / AZURE_GPT_IMAGE_15_ENDPOINT / AZURE_OPENAI_ENDPOINT." >&2
  exit 1
fi
