...
ALBEDO_API_KEY="${ALBEDO_API_KEY:-${AZURE_ALBEDO_OPENAI_API_KEY:-${AZURE_GPT_IMAGE_2_API_KEY:-${AZURE_GPT_IMAGE_15_API_KEY:-}}}}"
...
if [[ -z "${ALBEDO_ENDPOINT}" ]]; then
  echo "ALBEDO_ENDPOINT is not set (or AZURE_ALBEDO_OPENAI_ENDPOINT / AZURE_GPT_IMAGE_2_ENDPOINT / AZURE_GPT_IMAGE_15_ENDPOINT)." >&2
  exit 1
fi

if [[ -z "${ALBEDO_API_KEY}" ]]; then
  echo "ALBEDO_API_KEY is not set (or AZURE_ALBEDO_OPENAI_API_KEY / AZURE_GPT_IMAGE_2_API_KEY / AZURE_GPT_IMAGE_15_API_KEY)." >&2
  exit 1
fi
