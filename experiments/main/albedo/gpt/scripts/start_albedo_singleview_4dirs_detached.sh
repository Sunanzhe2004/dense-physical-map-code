...
if [[ -z "${ALBEDO_API_KEY:-}" && -z "${AZURE_ALBEDO_OPENAI_API_KEY:-}" && -z "${AZURE_GPT_IMAGE_15_API_KEY:-}" ]]; then
  echo "ALBEDO_API_KEY is not set (or AZURE_ALBEDO_OPENAI_API_KEY / AZURE_GPT_IMAGE_15_API_KEY)." >&2
  exit 1
fi

if [[ -z "${ALBEDO_ENDPOINT:-}" && -z "${AZURE_ALBEDO_OPENAI_ENDPOINT:-}" && -z "${AZURE_GPT_IMAGE_15_ENDPOINT:-}" ]]; then
